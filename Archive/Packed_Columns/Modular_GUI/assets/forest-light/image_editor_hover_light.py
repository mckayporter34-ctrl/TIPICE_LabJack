...
for x in range(width):
    for y in range(height):
        ...
        original_pixel = (36, 127, 76, 255)
        updated_pixel = (0, 126, 226, 255)
        new_pixel = []
        ...
        for i in range(len(current_pixel)-1):
            try:
                percent = np.abs(original_pixel[i] - current_pixel[i])/\
                    abs(original_pixel[i] - 255)
                dist = round(np.abs(updated_pixel[i] - 255) * percent)
                if original_pixel[i] > 255:
