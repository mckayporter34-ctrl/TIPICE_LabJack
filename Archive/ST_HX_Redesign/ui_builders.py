...
def build_sensor_display(parent, sensor: Sensor, row: int, col: int) -> dict:
    ...
    ttk.Label(parent, text=f"{sensor.label} ({sensor.unit})").grid(
        row=row, column=col, padx=5, pady=(10, 0), sticky="new"
    )
...
def build_control_loop_panel(
    ...
    Layout (matches the original packed-columns design):
      Column 0 : Setpoint spinbox, measured entry, optional extra-sensor entries
    ...
    # ── Left column: setpoint, measurement, optional extra sensor(s) ───────────
    ttk.Label(frame, text=f"{loop.label} Setpoint ({loop.unit})",
              wraplength=120).grid(
    ...
    ttk.Label(frame, text=f"{loop.label} ({loop.unit})",
              wraplength=120).grid(
    ...
    # Optional extra sensor readings embedded in this panel
    extra_keys = getattr(loop, "extra_sensor_keys", [])
    for idx, extra_key in enumerate(extra_keys):
        if extra_key in sensors:
            extra = sensors[extra_key]
            row_lbl = 4 + idx * 2
            row_ent = 5 + idx * 2
            ttk.Label(frame, text=f"{extra.label} ({extra.unit})",
                      wraplength=120).grid(
                row=row_lbl, column=0, padx=5, pady=(10, 0), sticky="nsew"
            )
            extra_entry = ttk.Entry(
                frame, textvariable=extra.value_var, state="readonly", width=10
            )
            extra_entry.grid(row=row_ent, column=0, padx=5, pady=(5, 10), sticky="ew")
            
            # Save references
            widgets[f"extra_entry_{idx+1}"] = extra_entry
            if idx == 0:
                widgets["extra_entry"] = extra_entry
            elif idx == 1:
                widgets["extra_entry_2"] = extra_entry

    # ── Center: manual override switch + slider ───────────────────────────────
    ...
    def _toggle_mode():
        ...
        if loop.is_auto:
            # Initialize integrator to current manual output so valve doesn't jump
            current_voltage = loop.get_manual_voltage()
            loop.pid.initialize(current_voltage)
            mode_switch.config(text="Mode: AUTO")
        else:
            loop.pid.reset()
            mode_switch.config(text="Mode: MANUAL")
