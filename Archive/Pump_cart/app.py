# app.py — Pump Cart GUI
# =======================
# Differences from packed_columns/app.py:
#   - No air MFC, no column selector relay
#   - Pump switch toggle in the controls frame
#   - Two control loop panels: flowrate (left) and pump speed (right)
#       - Flowrate panel shows main flow setpoint/reading + bypass flowrate
#       - Pump speed panel shows speed % setpoint/reading + actual RPM + motor power (W)
#   - Wide horizontal pressure panel spanning the full window width at the bottom
...
class PumpCartApp:
    ...
    def _build_ui(self):
        ...
        # Layout:
        #   col 0          col 1               cols 2-5
        #   ─────────────  ──────────────────  ──────────────────────────────
        #   Controls       Data Logging        Flowrate loop panel
        #   Logo           Pressures grid      Pump Speed loop panel

        # ── Col 0 ─────────────────────────────────────────────────────────────
        self._build_controls_frame()    # row=0, col=0
        self._build_logo_frame()        # row=1, col=0

        # ── Col 1 ─────────────────────────────────────────────────────────────
        self._build_data_logging_frame()   # row=0, col=1
        self._build_pressure_panel()       # row=1, col=1  (grid inside)

        # ── Cols 2-5: control loop panels ─────────────────────────────────────
        flowrate_widgets = build_control_loop_panel(
            self._sf, self.loops["flowrate"], self.sensors,
            row=0, col=2, columnspan=4, root=self.root,
        )
        self._loop_widgets = {"flowrate": flowrate_widgets}

        self._build_pump_speed_panel()     # row=1, cols 2-5
    ...
    def _build_logo_frame(self):
        """Logo and theme toggle — col 0, row 1."""
        f = tk.LabelFrame(self._sf, borderwidth=0, relief="flat")
        f.grid(row=1, column=0, padx=(20, 10), pady=(10, 10), sticky="nsew")
        ...
        ttk.Label(f, text="BYU TIPICE",
                  font=("Copperplate Gothic Bold", 40), wraplength=200).grid(
    ...
    def _build_data_logging_frame(self):
        """Data logging controls — col 1, row 0."""
        f = ttk.LabelFrame(self._sf, text="Data Logging", padding=(20, 10))
        f.grid(row=0, column=1, padx=(10, 10), pady=(10, 10), sticky="nsew")
        ...
        self._toggle_logging_btn.grid(
            row=0, rowspan=4, column=0, padx=20, pady=(20, 5), sticky="nsew"
        )
        ttk.Label(f, text="Interval (s)").grid(
    ...
    def _build_pump_speed_panel(self):
        """
        Build the pump speed control panel.
        Uses build_control_loop_panel for all standard PID widgets, then uses
        the returned frame reference to attach a second extra sensor (motor power W)
        directly below the first extra sensor (actual RPM).
        """
        widgets = build_control_loop_panel(
            self._sf, self.loops["pump_speed"], self.sensors,
            row=1, col=2, columnspan=4, root=self.root,
        )

        # Attach motor power as a second extra sensor inside the same panel frame.
        # extra_sensor (RPM) occupies rows 4-5 of column 0 inside the frame.
        # Motor power goes at rows 6-7 of column 0.
        panel_frame = widgets["frame"]
        motor_sensor = self.sensors["pump_motor_power"]
        ttk.Label(panel_frame,
                  text=f"{motor_sensor.label} ({motor_sensor.unit})").grid(
            row=6, column=0, padx=5, pady=(10, 0), sticky="nsew"
        )
        motor_entry = ttk.Entry(
            panel_frame, textvariable=motor_sensor.value_var,
            state="readonly", width=10,
        )
        motor_entry.grid(row=7, column=0, padx=5, pady=(5, 10), sticky="ew")
        widgets["extra_entry_2"] = motor_entry

        self._loop_widgets["pump_speed"] = widgets

    def _build_pressure_panel(self):
        """
        Pressure & temperature grid — col 1, row 1.
        Sensors are arranged in a grid with PRESSURE_PANEL_COLS columns.
        Adjust PRESSURE_PANEL_COLS in this method to change the grid width.
        Order is set by PRESSURE_PANEL_SENSORS in config.py.
        """
        PRESSURE_PANEL_COLS = 3    # sensors per row 

        f = ttk.LabelFrame(
            self._sf, text="Pressures & Temperature", padding=(10, 10)
        )
        f.grid(row=1, column=1, padx=(10, 10), pady=(10, 10), sticky="nsew")

        self._pressure_widgets = {}
        for idx, key in enumerate(config.PRESSURE_PANEL_SENSORS):
            if key not in self.sensors:
                continue
            grid_row = (idx // PRESSURE_PANEL_COLS) * 2   # *2 because each
            grid_col =  idx  % PRESSURE_PANEL_COLS        # sensor needs 2 rows
            w = build_sensor_display(f, self.sensors[key], row=grid_row, col=grid_col)
            self._pressure_widgets[key] = w
    ...
    def _disable_powered_controls(self):
        ...
        # Also turn pump off when main power goes off
        if self._pump_on:
            self._pump_on = False
