...
def build_control_loop_panel(
    ...
    ttk.Label(frame, text=f"{loop.label} Setpoint ({loop.unit})", wraplength=120).grid(
    ...
    ttk.Label(frame, text=f"{loop.label} ({loop.unit})",wraplength=120).grid(
        row=2, column=0, padx=5, pady=(10, 0), sticky="nsew"
    )
    measured_entry = ttk.Entry(
        frame, textvariable=loop.measured_var, state="disabled", width=5
    )
    ...
    if loop.extra_sensor_key and loop.extra_sensor_key in sensors:
        ...
        ttk.Label(frame, text=f"{extra.label} ({extra.unit})",wraplength=120).grid(
            row=4, column=0, padx=5, pady=(10, 0), sticky="nsew"
        )
        extra_entry = ttk.Entry(
            frame, textvariable=extra.value_var, state="readonly", width=5
        )
