...
class ShellTubeHXRedesignFrame(BaseAppFrame):
    ...
    def _create_sensor_overlays(self):
        ...
        overlays = [
            ...
            (545, 310, f"{lps['steam_pressure'].label} ({lps['steam_pressure'].unit})", lps['steam_pressure'].measured_var),
            (174, 378, f"{lps['steam_pressure'].label} Valve (%)", lps['steam_pressure'].rounded_valve_position),
            (570, 427, f"{s['tube_side_pressure_drop'].label} ({s['tube_side_pressure_drop'].unit})", s['tube_side_pressure_drop'].value_var),
            (1195, 57,  f"{s['makeup_temperature'].label} ({s['makeup_temperature'].unit})", s['makeup_temperature'].value_var),
            (1190, 140, f"{s['makeup_flowrate'].label} ({s['makeup_flowrate'].unit})", s['makeup_flowrate'].value_var),
            (1205, 378, f"{lps['level'].label} ({lps['level'].unit})", lps['level'].measured_var),
            (1165, 629, f"{lps['level'].label} Valve (%)", lps['level'].rounded_valve_position),
