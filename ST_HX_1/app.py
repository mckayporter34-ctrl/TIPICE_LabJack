...
class ShellTubeHXApp:
    ...
    def __init__(self):
        ...
        self._sensor_data   = {}   # key → latest engineering value
        self._loop_data     = {}   # key → latest measurement value
        self._loop_outputs  = {}   # key → latest output voltage
    ...
    def _build_logo_frame(self):
        ...
        ttk.Label(f, text="BYU TIPICE",
                  font=("IBM Plex Sans", 30), wraplength=180).grid(
    ...
    def _build_temp_panel(self):
        ...
        f = ttk.LabelFrame(self._sf, text="Temperatures", padding=(20, 5))
        f.grid(row=1, column=1, rowspan=2, padx=(10, 5), pady=(10, 0), sticky="new")
    ...
    def _build_pressure_panel(self):
        ...
        f = ttk.LabelFrame(self._sf, text="Pressures & Flow", padding=(20, 5))
        f.grid(row=1, column=2, rowspan=2, padx=(5, 10), pady=(10, 0), sticky="new")
    ...
    def _build_apparatus_image(self):

        f = ttk.LabelFrame(self._sf, text="Heat Exchanger #3", padding=(10, 10))
        f.grid(row=2, column=1, columnspan=2, padx=(10, 10), pady=(10, 10), sticky="nsew")
        f.rowconfigure(0, weight=1)
        f.columnconfigure(0, weight=1)

        base_dir = os.path.dirname(os.path.abspath(__file__))
        img_path = os.path.join(base_dir, "assets", "apparatus.png")
        try:
            img = Image.open(img_path)
            img = img.resize((200, 200))          # ← width, height in pixels
            self._apparatus_img = ImageTk.PhotoImage(img)
            ttk.Label(f, image=self._apparatus_img, anchor='center').grid(row=0, column=0, padx=5, pady=5,sticky='nsew')
        except Exception as e:
            print(f"Image load error: {e}")       # ← prints the actual error to terminal
    ...
    def _set_initial_states(self):
        ...
        for key_widgets in self._loop_widgets.values(): #comment out for testing
            disable_loop_widgets(key_widgets)
        #self._enable_powered_controls() ## Added for testing
    ...
    def _configure_ain_channels(self):
        """
        Write per-channel AIN settings defined in config.AIN_CONFIGS.
        Called once immediately after the LabJack connects.
        """
    ...
    def _schedule_updates(self):
        """Start background hardware thread and GUI refresh loop."""
        import time as _time
        self._hw_thread = Thread(target=self._hardware_loop, daemon=True)
        ...
        for key, loop in self.loops.items():
            ...
            try:
                if loop.is_auto:
                    ...
                    #print(f"[{key}] sp={sp} meas={meas}") #added 
