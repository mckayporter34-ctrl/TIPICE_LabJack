...
class CatalyticMethanationRedesignFrame(BaseAppFrame):
    ...
    def _build_pid_canvas(self, parent, row, col, columnspan):
        outer = ttk.LabelFrame(parent, text="P&ID — Catalytic Methanation Redesign", padding=(10, 10))
        ...
        try:
            ...
            bg_path = os.path.join(base_dir, "Catalytic_methanation_pid.png")
    ...
    def _build_simple_loop_panel(self, parent, key, loop, row, col):
        ...
        spinbox = ttk.Spinbox(slider_frame, from_=0, to=100, width=5,
                              textvariable=loop.rounded_valve_position, state="disabled")
        spinbox.grid(row=0, column=1, padx=(5, 0), sticky="w")
        widgets["manual_spinbox"] = spinbox

        def _sync_slider(*_a):
            try:
                if slider_frame.focus_get() != spinbox:
